...
# Initialize premium history for analytics
...
def show_health_tips():
    ...
    # Get health tips based on user's current metrics
...
def show_analytics():
    ...
    with col_demo:
        if st.button("🎯 Generate Sample Data (Demo)", help="Add sample historical data to see analytics"):
            generate_sample_data()
    
    with col_clear:
        if st.button("🗑️ Clear All Data", help="Clear all calculation history"):
            st.session_state.premium_history = []
            st.rerun()
...
def generate_sample_data():
    ...
    # Generate 7 sample data points over the past week
    base_date = datetime.now() - timedelta(days=7)
    
    for i in range(8):  # 8 data points including today
        date = base_date + timedelta(days=i)
        
        # Simulate improving wellness over time
        ...
        # Calculate premium using the actual models
        base_premium = st.session_state.ml_model.predict_premium(
